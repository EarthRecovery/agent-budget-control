...
ensure_conda_env() {
    ...
    if [ -z "${CONDA_DEFAULT_ENV:-}" ] || [ "${CONDA_DEFAULT_ENV}" != "ragenv2" ]; then
...
python -m pip install -U pip "setuptools>=77.0.3,<80.0.0" wheel
...
python -m pip install -c "${CONSTRAINTS_FILE}" beautifulsoup4 cleantext flask html2text rank_bm25 pyserini thefuzz gdown spacy rich
...
echo "To activate the environment, run: conda activate ragenv2"
